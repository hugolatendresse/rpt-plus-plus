...
Wall-time ratios match run_job.sh / existing columns: speedup_X_over_Y is time_X / time_Y
(same as speedup_3_over_4). Use case 2 as the reference column for 3-over-2 and 4-over-2.
...
for query_id in "${QUERY_IDS[@]}"; do
    ...
    if [[ "$case3_s" != "NA" && "$case4_s" != "NA" ]]; then
        ...
        speedup_3_over_4="$(ratio_time_over "$case3_s" "$case4_s")"
    fi

    if [[ "$case2_s" != "NA" && "$case3_s" != "NA" ]]; then
        speedup_3_over_2="$(ratio_time_over "$case3_s" "$case2_s")"
    fi

    if [[ "$case2_s" != "NA" && "$case4_s" != "NA" ]]; then
        speedup_4_over_2="$(ratio_time_over "$case4_s" "$case2_s")"
    fi
